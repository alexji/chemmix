...
if __name__=="__main__":
    ...
    if options.mmixgrid:
        ...
        mmixgrid = bgm.compute_mmix_grid(envname,numprocs=options.numprocs)
    ...
    if options.sfr:
        ...
        sfu.compute_umufns(envname,sfrname,tarr)
        print "Done! Time: %f" % (time.time()-start)
    if options.ckk:
        print "Computing ckk",envname,sfrname
        print "k2max: %i, k3max: %i" % (k2max,k3max)
        start = time.time()
        cgrid = bgm.compute_ckk(envname,sfrname,k2max,k3max,numprocs=options.numprocs)
    ...
    if options.fMkk:
        ...
        bgm.compute_fMkk(envname,sfrname,k2max,k3max,numprocs=options.numprocs)
